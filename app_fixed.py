...
def ask():
    ...
    if request.method == 'POST':
        ...
        key = f"{dept}_{course_number}"
        if key not in index:
            return "Course not found.<br><a href='/ask'>⬅️ Back</a>"
        syllabus_file = os.path.join(UPLOAD_FOLDER, index[key]['filename'])
        syllabus_text = extract_text_from_pdf(syllabus_file)
        context = f"Syllabus:\n{syllabus_text}\n\nQuestion: {question}"
        answer = talkToLLM(context)
    return render_template('ask_ui_final.html', index=index, question=question, answer=answer)
